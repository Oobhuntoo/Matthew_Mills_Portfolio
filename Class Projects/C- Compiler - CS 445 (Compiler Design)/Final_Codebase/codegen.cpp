/* author: Matthew Mills
   ...
   title: codegen.cpp
   purpose: Source file containing definition of functions used in code generation
   date: May 2020
*/
...
// DISCLAIMER: this file was not completed, so it will not run correctly and it is messy

void codeGeneration(TreeNode *tree, bool isTop, std::string filename, int mainAddr, SymbolTable st, int *globalOffset){
	
// code file pointer needs to be defined just once. At runtime, boolean flag is passed to codegeneration function as true so it passes the following if condition is initializes the file pointer. Then, the flag is set to false, so that the file pointer is not redefined in all successive recursive function calls
...
// the code generated for each node depends on the node's kind. Switch statement is used to determine node kind, and then code is generated accordingly using the formatting functions from emitcode.h to add the generated code to the tm assembly file that is run on the tiny machine emulator
